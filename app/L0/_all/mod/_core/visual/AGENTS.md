...
- `forms/dialog.css` owns the shared `.field.checkbox-field` variant for compact checkbox rows inside native dialog forms, overriding the default full-width text-input field treatment for those binary controls
