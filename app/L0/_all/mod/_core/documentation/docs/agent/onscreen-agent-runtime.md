...
- Codex CLI workspace, sandbox, model override, ephemeral flag, and git-repo-check flag
- `max_tokens`
...
The settings dialog now has three provider tabs named `API`, `Local`, and `Codex CLI`. `API` keeps the OpenAI-compatible endpoint, model, and key fields. `Local` mounts the shared Hugging Face config sidebar in onscreen mode, so the overlay reads the same saved-model list and live WebGPU worker state as the routed Local LLM page and the admin chat. `Codex CLI` configures the authenticated server-owned local Codex transport through `/api/codex_chat`, including workspace, sandbox, optional model override, ephemeral execution, and git-repo-check behavior. Opening the Local tab should refresh saved-model shortcuts without booting the worker; saving local settings persists the selected repo id and dtype, then starts background model preparation. When no local model is selected and saved models exist, the Local panel preselects the browser-wide last successfully loaded saved model from `_core/huggingface/manager.js`, falling back to the first saved entry if that last-used entry was discarded. When no local model is selected, no local model is loaded, and the shared saved-model list is empty, the Local panel prefills the Hugging Face model field with the same testing-page default: `onnx-community/gemma-4-E4B-it-ONNX`.

The API-key composer blocker applies only to the default API-provider configuration with no API key, where the composer shows a centered `Set LLM API key` action over the disabled textarea. Local Hugging Face mode can send without an API key and falls back to loading the selected local model on the first message if background preparation has not finished. Codex CLI mode can also send without an API key because the authenticated server endpoint owns the local `codex exec --json` process and validates the requested workspace and sandbox before spawning it.
...
Local Hugging Face sends and Codex CLI sends use the same onscreen-agent prompt assembly path as remote API sends, so the firmware prompt, prompt includes, skill catalog, auto-loaded skill context, custom instructions, history, and transient context stay aligned across providers. Hugging Face reuses the folded transport messages that the API path would send upstream. Codex CLI sends those prepared messages to `/api/codex_chat`, which wraps them into a local Codex prompt and adapts `codex exec --json` JSONL events back into OpenAI-compatible SSE chunks. Prompt inspection keeps showing the richer pre-fold prepared payload. The routed `/huggingface` testing page remains separate and uses only its local plain system-prompt-plus-chat surface.
