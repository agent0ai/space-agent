...
- `onscreen_agent/`: floating routed overlay agent and the first-party user-facing chat runtime, including API, browser-local Hugging Face, and server-owned Codex CLI provider branches
...
- `open_router/`: headless OpenRouter request-policy module that extends the admin and onscreen API transport seams instead of hardcoding provider-specific headers into the chat runtimes; Codex CLI is not an API-upstream policy and instead routes through `/api/codex_chat`
