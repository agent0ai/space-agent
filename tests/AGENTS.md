...
- `codex_cli_provider_test.mjs`: focused server-side coverage for the local Codex CLI provider prompt wrapper, endpoint error boundaries, fake-CLI `codex exec --json` argument shape, and OpenAI-compatible SSE adaptation without requiring live Codex credentials
